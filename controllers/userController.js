...
const { userId } = req.body; // user ID from frontend (AuthContext)
...
try {
  await db.promise().query(
    `INSERT INTO saved_jobs (user_id, job_id, status) 
      ...
      status = 'saved', updated_at = CURRENT_TIMESTAMP`,
    [userId, jobId]
  );
...
export const unsaveJob = async (req, res) => {
  const { jobId } = req.params;
  const { userId } = req.body; // ✅ from body (matches frontend + saveJob)
...
export const getSavedJobs = async (req, res) => {
  const { userId } = req.query; // frontend: /api/saved-jobs?userId=xx
